...
--The first_name should be ordered in ascending order--
 SELECT first_name, last_name, COUNT(*) AS count FROM owners JOIN vehicles ON owners.id = vehicles.owner_id
...
SELECT first_name, last_name, COUNT(owner_id) AS count, ROUND(AVG(price)) AS average_price FROM owners JOIN vehicles
ON owners.id = vehicles.owner_id  GROUP BY (first_name,last_name) HAVING COUNT(owner_id) > 1 AND ROUND(AVG(price)) > 10000
